({
    ...
    return this.SLDSPromise(function(resolve,reject) {
      ...
      action.setCallback(this, function(response) {
        ...
        if (status === "SUCCESS") {
          ...
          toastComponent.showToast($A.get("$Label.c.Toast_Title"), errors);
        }
      });
  ...
  navigateIndex: function(cmp, index) {
    ...
    window.location.assign('/apex/'+type.replace('c:','')+'?mo='+masterObject+'&sid='+solutionId+'&sn='+solutionName+'&theme='+theme);
    ...
    showToast : function(cmp, message) {
      ...
      toastComponent.showToast($A.get("$Label.c.Toast_Title"), message);
    },  
